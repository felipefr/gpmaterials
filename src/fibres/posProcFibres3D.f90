...
!~ 
Subroutine posProcFibres3D(AE, BE, MaxLRows, XLL, NDim, iDofT, NodElt, Sol0, Sol1, CommonPar, Param, JParam, DelT, DTm, Time)
!     ------------------------------------------------------------------
	use funcAux
...
!~ 	damagePar(3) = 3.00001d0
	...
	if(Nfields < 1 ) then !! default mode
		...
		Param(LengthParam + Ipos_stress : LengthParam + Ipos_stress + NdimE - 1 ) = Sfib 

		Param(Ipos_stretch) = norm2(qfib)
		Param(Ipos_normStress) = dsign(1.0d0,dot_product(Sfib,afib))*norm2(Sfib)
		Param(Ipos_stress : Ipos_stress + NdimE - 1 ) = Sfib 
	else
		do i = 1 , NFields
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Subroutine posProcFibres3DS(Coupling,CommonPar,iDofT,Ndim,MaxLRows,iAdd)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	IMPLICIT NONE
