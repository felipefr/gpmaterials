...
Subroutine computeCoeficientsArcLength_generic(AE, BE, MaxLRows, XLL, NDim, iDofT, NodElt, & 
...
!~ 	!   =====   END ARGUMENTS  =======
	...
	real*8, allocatable, dimension(:) ::  Sol_dUf_Star, Sol_dUf_Bar , Sol_DUf_acc, Xel , SolGy, Sol_DU_acc, &
										  Sol_dLp_Star, Sol_dLp_Bar , Sol_DLp_acc
	...
	integer :: 	iShift_c1, iShift_c2, iShift_c3, iShift_dUf_Star, iShift_dUf_Bar, & 
				iShift_DUf_acc, iShift_dLarcLength_acc, iShift_dLp_Bar, iShift_dLp_Star, iShift_DLp_acc
	...
	iShift_dLp_Star = nint(CommonPar(9))
	iShift_dLp_Bar = nint(CommonPar(10))
	iShift_DLp_acc = nint(CommonPar(11))
	iShift_dLarcLength_acc = nint(CommonPar(12))
	iFEMtype =  nint(commonPar(13)) 
	iFtype = nint(commonPar(14)) 
	kindArcLength = nint(commonPar(15))
	yG = commonPar(16:17)
	...
	call getSliceAllocate(Sol_dLp_Bar,Sol1,1,NodG, iShift_dLp_Bar + 1 , iShift_dLp_Bar + NdimE, iDofT)
	call getSliceAllocate(Sol_dLp_Star,Sol1,1,NodG, iShift_dLp_Star + 1 , iShift_dLp_Star + NdimE, iDofT)
	call getSliceAllocate(Sol_DLp_acc,Sol1,1,NodG, iShift_DLp_acc + 1 , iShift_DLp_acc + NdimE, iDofT)
	...
	if(kindArcLength == 1) then
		...
		c1 = c1 + psi*psi*dot_product(Sol_dLp_Bar,Sol_dLp_Bar)
		c2 = c2 + 2.0d0*psi*psi*dot_product(Sol_dLp_Bar,Sol_DLp_acc + Sol_dLp_Star)
		c3 = c3 + psi*psi*dot_product(Sol_DLp_acc + Sol_dLp_Star,Sol_DLp_acc + Sol_dLp_Star)
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Subroutine computeCoeficientsArcLength_genericS(Coupling,CommonPar,iDofT,Ndim,MaxLRows,iAdd)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    use funcAux
    use ptsGaussLib
    IMPLICIT NONE
